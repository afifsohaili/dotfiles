...
```bash
# 1. Check server running (app runs on portless HTTPS)
curl -sk -o /dev/null -w "%{http_code}" https://app.excalidraw.localhost
# 200 = OK, else ask user to run `pnpm dev` + open browser
...
If no files listed: user must open https://app.excalidraw.localhost in browser.
If commands timeout: ask user to refresh browser page.
If TLS verification fails: run `portless trust`, or bypass with `-u ws://localhost:<port>/api/ws`.
If the server sets `EXCALIDRAW_AUTH_TOKEN`: pass `--token <value>` or set the same env var.
...
| `read -f ID --active` | List non-deleted elements (`--json` for full JSON) |
...
| `resize-frame -f ID -i name` | Resize frame to fit its children |
...
# Arrows auto-bind to elements (follow when moved); elbowed by default
```
...
# Add to existing frame (auto-resizes it):
excalidraw-cli add-rectangle -f <ID> -x 100 -y 100 --name newbox --in-frame myframe
# Refit frame after manual edits:
excalidraw-cli resize-frame -f <ID> -i myframe
```
...
### Delete
```bash
excalidraw-cli delete -f <ID> -i box1 box2 box3  # accepts names or UUIDs; bound text is deleted too
```
...
| `--gap <n>` | Spacing (default: 50 adjacent, 20 rectangle inside, 15 text inside) |
...
4. **Names work everywhere**: `-i`, `--from`, `--to`, `--right-of`, etc. (stored in `customData.name`)
